...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    private lazy var character: CharacterStore = CharacterStore(urlSession: urlSession)
    ...
    func scene(_ scene: UIScene, willConnectTo session: UISceneSession, options connectionOptions: UIScene.ConnectionOptions) {
        ...
        let vc = UIStoryboard(name: "Main", bundle: nil).instantiateViewController(identifier: "ViewController", creator: { coder in
            return ViewController(coder: coder, viewModel: ViewModel(imageStore: ImageStore(urlSession: self.urlSession), getCharacter: self.character.getCharacters))
        })
